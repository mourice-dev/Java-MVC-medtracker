    ...
    <div class="flex h-screen bg-slate-50">
        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden">
            <jsp:include page="components/header.jsp">
                <jsp:param name="searchAction" value="appointments" />
                <jsp:param name="searchPlaceholder" value="Search appointments..." />
            </jsp:include>
            <main class="flex-1 overflow-y-auto p-6 bg-slate-50">
                <div class="max-w-7xl mx-auto space-y-6">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                        <h2 class="text-2xl font-bold text-slate-800">Appointment Management</h2>
                        <button onclick="openModal()" class="bg-teal-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-teal-700 shadow-lg shadow-teal-600/20 flex items-center justify-center">
                    ...
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        <form action="appointments" method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <% 
                                String filterDate = request.getParameter("searchDate");
                            ...
                            <div>
                                ...
                                <input type="date" name="searchDate" value="<%= filterDate != null ? filterDate : "" %>" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Status</label>
                                <select name="status" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white">
                                    <option value="">All Statuses</option>
                                    <option value="Scheduled" <%= "Scheduled".equals(filterStatus) ? "selected" : "" %>>Scheduled</option>
                            ...
                            <div>
                                ...
                                <select name="type" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white">
                                    <option value="">All Types</option>
                                    <option value="Consultation" <%= "Consultation".equals(filterType) ? "selected" : "" %>>Consultation</option>
                                    <option value="Follow-up" <%= "Follow-up".equals(filterType) ? "selected" : "" %>>Follow-up</option>
                                    <option value="Lab Review" <%= "Lab Review".equals(filterType) ? "selected" : "" %>>Lab Review</option>
                                    <option value="Emergency" <%= "Emergency".equals(filterType) ? "selected" : "" %>>Emergency</option>
                                </select>
                            </div>
                            <div class="flex items-end gap-2">
                                <button type="submit" class="flex-1 bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-900 transition">Filter</button>
                                <a href="appointments" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 transition">Reset</a>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        if (appointments != null && !appointments.isEmpty()) { 
                            for (Appointment apt : appointments) { 
                                ...
                                if (apt.getAppointmentTime() != null) {
                                    timeStr = timeFormat.format(apt.getAppointmentTime());
                                }
                        %>
                        <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col hover:border-teal-200 transition duration-200">
                            ...
                            <p class="text-slate-500 text-sm mb-4">
                                <%= apt.getType() != null ? apt.getType() : "General" %>
                            </p>
                            <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                                <div class="flex items-center text-sm text-slate-600">
                                    <i data-lucide="stethoscope" class="w-4 h-4 mr-2 text-teal-600"></i>
                                    <%= apt.getDoctorName() != null ? apt.getDoctorName() : "Unassigned" %>
                                </div>
    ...
    <div id="appointmentModal" class="fixed inset-0 bg-black bg-opacity-50 <%= editMode ? "flex" : "hidden" %> items-center justify-center z-50">
        <div class="bg-white rounded-xl p-6 w-full max-w-lg mx-4">
            ...
            <form action="appointments" method="post">
                ...
                <% if (editMode && editAppointment != null) { %>
                <input type="hidden" name="id" value="<%= editAppointment.getId() %>">
                <% } %>
                <div class="space-y-4">
                    <div>
