    ...
    <div class="flex h-screen bg-slate-50">
        ...
        <div class="flex-1 flex flex-col h-full overflow-hidden">
            <jsp:include page="components/header.jsp">
                <jsp:param name="searchAction" value="staff" />
                <jsp:param name="searchPlaceholder" value="Search staff..." />
            </jsp:include>
            <main class="flex-1 overflow-y-auto p-6 bg-slate-50">
                <div class="max-w-7xl mx-auto space-y-6">
                    <!-- Search & Filters -->
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        <form action="staff" method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <% 
                                String filterRole = request.getParameter("role");
                                String filterDept = request.getParameter("department");
                            ...
                            <div>
                                ...
                                <select name="role" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white">
                                    <option value="">All Roles</option>
                                    <option value="DOCTOR" <%= "DOCTOR".equals(filterRole) ? "selected" : "" %>>Doctor</option>
                                    <option value="NURSE" <%= "NURSE".equals(filterRole) ? "selected" : "" %>>Nurse</option>
                            ...
                            <div>
                                ...
                                <input type="text" name="department" value="<%= filterDept != null ? filterDept : "" %>" placeholder="e.g. Cardiology" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500">
                            </div>
                            <div>
                                ...
                                <select name="status" class="w-full p-2 border rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white">
                                    ...
                                    <option value="On Duty" <%= "On Duty".equals(filterStatus) ? "selected" : "" %>>On Duty</option>
                                    <option value="In Surgery" <%= "In Surgery".equals(filterStatus) ? "selected" : "" %>>In Surgery</option>
                                    <option value="On Break" <%= "On Break".equals(filterStatus) ? "selected" : "" %>>On Break</option>
                                    <option value="Off Duty" <%= "Off Duty".equals(filterStatus) ? "selected" : "" %>>Off Duty</option>
                                </select>
                            </div>
                            <div class="flex items-end gap-2">
                                ...
                                <a href="staff" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 transition">Reset</a>
                            </div>
                        </form>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                            ...
                            if (staffList != null && !staffList.isEmpty()) { 
                                for (Staff staff : staffList) { 
                                    ...
                                    if ("Active".equals(status) || "On Duty".equals(status)) {
                                        statusColor = "bg-green-100 text-green-700";
                                    } else if ("In Surgery".equals(status)) {
                                        statusColor = "bg-purple-100 text-purple-700";
                                    } else if ("On Break".equals(status)) {
                                        statusColor = "bg-orange-100 text-orange-700";
                                    } else if ("Off Duty".equals(status) || "Inactive".equals(status)) {
                                        statusColor = "bg-gray-100 text-gray-700";
                                    }
                                    String initial = "?";
                                    if (staff.getFullName() != null && staff.getFullName().length() > 0) {
                                        initial = staff.getFullName().substring(0, 1).toUpperCase();
                                    }
                            %>
